...
repo_root = strtrim(getenv('MATERNAL_PROJECT_ROOT'));
if isempty(repo_root)
    % toolbox is <project>/code/subepidemic/toolbox
    repo_root = fileparts(fileparts(fileparts(toolbox_dir)));
end
switch caddisease
    case 'mmr'
        data_file = fullfile(repo_root, 'data', 'source', 'mmeig', ...
            'yearly-mmr-deaths-world-01-01-2023.txt');
    case 'maternal'
        data_file = fullfile(repo_root, 'data', 'source', 'mmeig', ...
            'yearly-maternal-deaths-world-01-01-2023.txt');
    otherwise
        error('Unsupported manuscript workflow caddisease value: %s', caddisease);
end
if ~isfile(data_file)
    error('Input data file not found: %s', data_file);
end
