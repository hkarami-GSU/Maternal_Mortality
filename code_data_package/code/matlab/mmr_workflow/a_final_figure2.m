...
output_filename_suffix = '_mmr.pdf';
...
try
    mmr_data = readmatrix('transformed_ratio_data.xlsx');
catch ME
...
for i=1:numAreas
    ...
    dist=results(area);
    ensembleFilename=sprintf('%s/Ensemble(4)-onsetfixed-0-flag1-1-method-0-dist-%d-horizon-4-weighttype-0-yearly-mmr-deaths-world-area-%d-01-01-2019.csv',output_folder,dist,area);
    rankedFilename=sprintf('%s/ranked(1)-onsetfixed-0-flag1-1-method-0-dist-0-horizon-4-yearly-mmr-deaths-world-area-%d-01-01-2019.csv',output_folder,area);
...
for i=1:numAreas
    ...
    try
        ...
        if mod(subplot_counter-1,numCols)==0
            ylabel('MMR','Interpreter','latex','FontSize',labelFontSize);
        end
