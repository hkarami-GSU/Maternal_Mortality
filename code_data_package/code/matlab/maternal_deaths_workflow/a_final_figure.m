...
if selectedIndex==8
    highBurdenCountries = {'Bangladesh','China','India','Indonesia','Nigeria',...
                          'DRC','Philippines','Pakistan'};
...
try
    ...
    error('Could not read transformed_maternal_deaths_data.xlsx: %s',ME.message);
end
...
figure('Position',[100,100,1200,200*actualNumRows]);
subplot_counter=0;
years=(2000:2023)';
titleFontSize=22; labelFontSize=20; tickFontSize=18;
for i=1:numAreas
    area=selectedAreas(i);
    ...
    try
        ...
        hold on;
        plot(years(1:calibration_period),actual_data(1:calibration_period),'o','MarkerEdgeColor','k','MarkerFaceColor','none');
        plot(years(forecast_start_idx:end),mmr_forecast_data,'o','MarkerEdgeColor',[0 0.5 0],'MarkerFaceColor',[0 0.5 0]);
        plot(years,median_data,'r-','LineWidth',1.7);
        plot(years,lower_bounds_all,'k--','LineWidth',1.2);
        plot(years,upper_bounds_all,'k--','LineWidth',1.2);
        xlim([2000,2023]);
        ylims=ylim;
        ...
        patch([2019,2023,2023,2019],[ylims(1),ylims(1),ylims(2),ylims(2)],[0.7,0.7,0.7],'FaceAlpha',0.3,'EdgeColor','none','Clipping','off');
        uistack(findobj(gca,'Type','patch'),'bottom');
        xline(2019,'b--','LineWidth',1.2);
        ...
        if subplot_counter>(actualNumPlots-numCols), xticks([2000,2008,2015,2023]); else xticks([]); end
        if mod(subplot_counter-1,numCols)==0, ylabel('Maternal Deaths','Interpreter','latex','FontSize',labelFontSize); end
...
if actualNumPlots>0
    widthPerCol=5; heightPerRow=3;
    totalWidth=numCols*widthPerCol; totalHeight=actualNumRows*heightPerRow;
    fig=gcf;
    fig.PaperUnits='inches';
    fig.PaperPosition=[0,0,totalWidth,totalHeight];
    fig.PaperSize=[fig.PaperPosition(3),fig.PaperPosition(4)];
    output_filename = sprintf('%s_maternaldeaths.pdf', selectedRegion);
