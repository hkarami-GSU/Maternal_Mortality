clc; clear; close all
output_folder='output'; figures_folder='excess_deaths_figures'; calibration_period=20; results=zeros(195,1);
...
disp("Select a region:"); for i=1:length(regions), fprintf("%d: %s\n",i,regions{i}); end
while true, selectedIndex=input("Enter number: "); if isnumeric(selectedIndex)&&selectedIndex>=1&&selectedIndex<=length(regions), break; end, fprintf("Invalid selection.\n"); end
selectedRegion=regions{selectedIndex};
if selectedIndex==8, highBurdenCountries = {'Afghanistan','South Sudan','Chad','Nigeria','Guinea-Bissau','Liberia','Somalia','Lesotho'}; filteredData=classificationData(ismember(classificationData{:,2},highBurdenCountries),:); else, filteredData=classificationData(strcmp(classificationData{:,3},selectedRegion),:); end
...
if ~exist(output_folder,'dir'), warning('Output folder "%s" does not exist.',output_folder); return; end
maternal_deaths_data=readmatrix('transformed_maternal_deaths_data.xlsx');
numAreas=length(selectedAreas); existingFiles=false(1,numAreas); existingFilenames=cell(1,numAreas); fileTypes=cell(1,numAreas);
for i=1:numAreas
    area=selectedAreas(i); dist=results(area);
    ...
    if exist(ensembleFilename,'file'), existingFiles(i)=true; existingFilenames{i}=ensembleFilename; fileTypes{i}='ensemble';
    elseif exist(rankedFilename,'file'), existingFiles(i)=true; existingFilenames{i}=rankedFilename; fileTypes{i}='ranked';
    else, existingFiles(i)=false; existingFilenames{i}=''; fileTypes{i}=''; end
end
actualNumFiles=sum(existingFiles); if actualNumFiles==0, error('No CSV files found.'); end
...
for i=1:numAreas
    ...
    if area<=size(maternal_deaths_data,2)&&size(maternal_deaths_data,1)>=max(forecast_indices), maternal_deaths_forecast_data=maternal_deaths_data(forecast_indices,area+1); else, continue; end
    ...
    for j=1:length(years_to_plot), current_year=years_to_plot(j); year_idx=find(years==current_year);
        if ~isempty(year_idx), excel_value=maternal_deaths_forecast_data(j);
...
for year_idx=1:length(years_to_plot)
    ...
    xlabel(sprintf('Excess Maternal Deaths in %d',current_year),'FontSize',12,'FontWeight','bold','Interpreter','latex');
    ...
    exportname=sprintf('%s/ExcessMaternalDeaths_%s_%d',figures_folder,selectedRegion,current_year);
...
if ~isempty(valid_countries)
    ...
    errorbar(sorted_medians,y_positions,err_neg,err_pos,'horizontal','.k','LineWidth',1.5);
    ylim([0.5,num_countries+0.5]); yticks(y_positions); yticklabels(sorted_countries);
    xlabel(sprintf('Total Excess Maternal Deaths (%d-%d)',years_to_plot(1),years_to_plot(end)),'FontSize',12,'FontWeight','bold','Interpreter','latex');
    max_data_value=max(sorted_uppers); padding=max_data_value*0.02; xlim([0,max_data_value*1.3]);
    for i=1:num_countries, text(sorted_uppers(i)+padding,y_positions(i),sprintf('$%d \\, [%d\\mbox{--}%d]$',round(sorted_medians(i)),round(sorted_lowers(i)),round(sorted_uppers(i))),'VerticalAlignment','middle','HorizontalAlignment','left','FontSize',10,'Interpreter','latex'); end
    set(gca,'Position',[0.2,0.08,0.55,0.88],'FontSize',10,'TickLength',[0 0]);
    exportname=sprintf('%s/ExcessMaternalDeaths_%s_Total',figures_folder,selectedRegion);
    exportgraphics(fig,exportname+".pdf",'ContentType','vector','BackgroundColor','none','Resolution',800); hold off; close(fig);
end
...
excel_output=sprintf('%s/ExcessMaternalDeaths_%s_Formatted.xlsx',figures_folder,selectedRegion); writetable(formattedTable,excel_output);
