% MATLAB Code to Transform CSV Data to Year-Country Matrix
% Input: CSV file with columns: Indicator, Location, Period, FactValueNumeric
...
function transformCSVData()
    ...
    try
        ...
        output_filename = 'transformed_maternal_deaths_data.csv';
        writetable(output_table, output_filename);
        fprintf('\nTransformed data saved to: %s\n', output_filename);
        
        % Optional: Save as Excel for easier viewing
        excel_filename = 'transformed_maternal_deaths_data.xlsx';
        writetable(output_table, excel_filename);
        fprintf('Also saved as Excel: %s\n', excel_filename);
        ...
        fprintf('Please ensure the file "number.csv" exists in the current directory.\n');
